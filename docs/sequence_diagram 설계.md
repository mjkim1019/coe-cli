...
``` 
@startuml
title Mider CLI - Add Mode Sequence Diagram

actor User
participant "Mider CLI" as CLI
participant "FileManager" as FM
participant "MiderAnalyzer\n(Mider)" as CA
...
파일 확장자(C, SQL, XML 등)에 따라 MiderAnalyzer를 이용한 **기본 구조 분석**을 수행합니다.
...
```
@startuml
title Mider CLI - Ask Mode Sequence Diagram
...
2. 파일 컨텍스트 및 기본 구조 분석 추가
   ...
   - MiderAnalyzer를 사용해 기본 구조 분석 처리한 내용 또한 system 메세지로 추가합니다.
3. 조건부 심화 분석 처리
   - 사용자 질문에 “분석” 키워드가 포함된 경우에만, MiderAnalyzer를 통해 LLM 기반 심화 분석(use_llm=true) 을 수행합니다.
